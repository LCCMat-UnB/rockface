...
LCCMat thin-section preparation) in a patch's "normal" composite image
and produces a transparent overlay suitable for a digital mosaic
viewer.
...
def composite_overlay_on_image(base_rgb: np.ndarray, overlay_rgba: np.ndarray) -> np.ndarray:
    ...
    Args:
        base_rgb: The underlying image to draw on top of (``HxWx3``,
            ``uint8``, RGB order) -- typically a "normal" composite
            patch or a restitched mosaic, already converted via
...
def _process_patch_worker(
    ...
    Loads a source patch array (normally ``*_normal.npy``, or a
    single-channel ``*_pol{c}.npy`` fallback -- see
    :func:`rockface.patching.find_normal_source`), segments it, and
    saves whichever of ``formats`` were requested as ``{base}_mask.*``
    -- ``base`` derived by stripping the trailing ``_normal`` or
    ``_pol{c}`` suffix from the source filename, so the mask is always
    named after its patch coordinate alone (``patch_y{y}_x{x}_mask.*``),
    regardless of which source variant produced it.
    ...
    base = re.sub(r"_(normal|pol\d+)$", "", source_npy_path.stem)

    try:
        normal_array = np.load(source_npy_path)
        mask = generate_pore_mask_array(normal_array)
...
class Mask:
    ...
    def overlay_on_patch(
        ...
        base_rgb = convert_array_to_rgb(normal_array, pixel_format=pixel_format)
    ...
    def run(
        ...
        from .patching import _PATCH_COORD_RE, find_normal_source
        ...
        if coords is not None:
            normal_files = []
            for y, x in coords:
                candidate = find_normal_source(patch_dir, y, x)
                if candidate is not None:
                    normal_files.append(candidate)
                else:
                    missing_count += 1
                    logger.warning(
                        "No extracted patch (normal or single-channel fallback) found "
                        "for mask request (%d, %d) in %s", y, x, patch_dir,
                    )
        else:
            # Every *_normal.npy, plus -- for patches extracted with only
            # one channel (no _normal file written, see find_normal_source)
            # -- their single *_pol{c}.npy file used as a stand-in.
            seen_coords = set()
            normal_files = []
            for path in patch_dir.glob("*_normal.npy"):
                match = _PATCH_COORD_RE.match(path.name)
                if match:
                    seen_coords.add((int(match.group(1)), int(match.group(2))))
                normal_files.append(path)
            for path in patch_dir.glob("*_pol*.npy"):
                match = _PATCH_COORD_RE.match(path.name)
                if not match:
                    continue
        ...
        with multiprocessing.Pool(workers) as pool:
            for i, success in enumerate(pool.imap_unordered(worker_fn, normal_files), 1):
                if success:
                    ok_count += 1
                else:
                    failed_count += 1
